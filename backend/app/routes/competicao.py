...
def create_competicao(body: CompeticaoCreate, db: Session = Depends(get_db), _=Depends(require_admin)):
    return criar_competicao(db, body)

@router.get("", response_model=list[CompeticaoResponse])
def get_competicoes(db: Session = Depends(get_db)):
    return listar_competicoes(db)

@router.get("/{competicao_id}", response_model=CompeticaoResponse)
def get_competicao(competicao_id: int, db: Session = Depends(get_db)):
...
def update_competicao(competicao_id: int, body: CompeticaoUpdate, db: Session = Depends(get_db), _=Depends(require_admin)):
    obj = buscar_competicao(db, competicao_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Competição não encontrada.")
...
def delete_competicao(competicao_id: int, db: Session = Depends(get_db), _=Depends(require_admin)):
    obj = buscar_competicao(db, competicao_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Competição não encontrada.")
