...
def create_temporada(body: TemporadaCreate, db: Session = Depends(get_db), _=Depends(require_admin)):
...
def update_temporada(temporada_id: int, body: TemporadaUpdate, db: Session = Depends(get_db), _=Depends(require_admin)):
    obj = buscar_temporada(db, temporada_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Temporada não encontrada.")
...
def delete_temporada(temporada_id: int, db: Session = Depends(get_db), _=Depends(require_admin)):
    obj = buscar_temporada(db, temporada_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Temporada não encontrada.")
